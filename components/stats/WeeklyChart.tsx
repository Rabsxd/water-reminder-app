  ...
  ({
    ...
    React.useEffect(() => {
      if (animated && animatedValues.current.length === 0) {
        animatedValues.current = Array(7)
          .fill(0)
    ...
    const maxIntake = Math.max(
      ...data.map((d) => Math.max(d.intake, d.target)),
      2000
    );
    const barWidth = Math.max(
      20,
      (screenWidth - Number(SPACING) * 4) / data.length - Number(SPACING)
    );
        ...
        {data.every((d) => d.intake === 0) && (
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptyStateSubtext}>
              Start logging your water intake on the Home screen to see your
